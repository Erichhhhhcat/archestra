...
describe("persistTools", () => {
  test("creates new tools and agent-tool relationships in bulk", async ({
    ...
    const agentTools = await ToolModel.getToolsByProfile(agent.id);
  ...
  test("handles empty tools array without errors", async ({ makeProfile }) => {
    const agent = await makeProfile({ name: "Test Agent" });
    ...
    const proxyTools = agentTools.filter(
      (t) => t.profileId === agent.id && t.catalogId === null,
    );
    expect(proxyTools).toHaveLength(0);
  });

  test("skips Archestra built-in tools", async ({ makeProfile }) => {
    const agent = await makeProfile({ name: "Test Agent" });
  ...
  test("skips agent delegation tools (agent__*)", async ({ makeProfile }) => {
    const agent = await makeProfile({ name: "Test Agent" });
  ...
  test("skips MCP tools already assigned to the agent", async ({
    ...
    await ProfileToolModel.createIfNotExists(agent.id, mcpTool.id);
  ...
  test("handles tools with missing optional fields", async ({
    makeProfile,
  }) => {
    const agent = await makeProfile({ name: "Test Agent" });
    ...
    const proxyToolNames = agentTools
      .filter((t) => t.profileId === agent.id && t.catalogId === null)
      .map((t) => t.name);
  ...
  test("handles concurrent calls without errors", async ({ makeProfile }) => {
    const agent = await makeProfile({ name: "Test Agent" });
  ...
  test("filters all tools when all are MCP or Archestra tools", async ({
    ...
    // Get count before
    const toolsBefore = await ToolModel.getToolsByProfile(agent.id);
    ...
    const toolsAfter = await ToolModel.getToolsByProfile(agent.id);
